#!/usr/bin/env bash
# Fresh E3c development diagnostic only. No qualification/confirmation/test capability.
set -euo pipefail
...
RUN_PARENT="results/e3c_output_contract/qwen"
RUN_DIR="${RUN_PARENT}/run"
CONFIG="configs/experiments/e3c_output_contract.json"
ARTIFACT="/kaggle/working/hibermem-e3c-qwen-${HIBERMEM_REF:0:12}-$(date -u +%Y%m%dT%H%M%SZ)-$$.tar.gz"
...
PY
  ...
  echo "Exit ${status}: 0=validated development diagnostic; 2=setup/test/runtime error."
  echo "No contract is selected automatically. Confirmation and test remain locked."
  exit "${status}"
}
trap 'finish "$?"' EXIT

BASE_TORCH="$("${BASE_PYTHON}" -c 'import torch; print(torch.__version__)')"
ENV_DIR="/kaggle/working/hibermem-e3c-env-nopip"
...
STAGE="controls"
"${ENV_PYTHON}" scripts/run_e3c_contract.py --config "${CONFIG}" --controls-only \
  2>&1 | tee "${RUN_PARENT}/controls.log"
STAGE="diagnostic"
"${ENV_PYTHON}" -u scripts/run_e3c_contract.py --config "${CONFIG}" \
  --candidate qwen --run-dir "${RUN_DIR}" 2>&1 | tee "${RUN_PARENT}/run.log"
STAGE="artifact_validation"
"${ENV_PYTHON}" scripts/validate_e3c_report.py --report "${RUN_DIR}/report.json"
VALIDATED=1
STAGE="complete"
